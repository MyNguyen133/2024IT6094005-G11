...
python train.py --weights yolov5s.pt --data data.yaml --epochs 50 --imgsz 640 --batch-size 16 --project runs/train --name yolov5_license_plate --exist-ok
    Lệnh chạy huấn luyện lại
python train.py --weights runs/train/yolov5_license_plate/weights/last.pt --data data.yaml --epochs 50 --imgsz 640 --batch-size 16 --project runs/train --name yolov5_license_plate --exist-ok
    Lệnh chạy đoán
python detect.py --weights best.pt --data data.yaml --source imgs --imgsz 640 --conf-thres 0.4 --save-txt --save-conf --save-crop --project ./runs/detect --name detected --exist-ok
    Lệnh chạy xử lý vùng cắt
python process_lp.py
'''
